...
class Pipeline:
    ...
    def generate_prompt(self, schema):
        ...
        for chunk in self.chunks:
            ...
            for index, row in chunk.iterrows():
                document = document + str(row['segment']) + ' ' + row['segment_id'] + '\n'
    ...
    def image_from_pdf(self, file_path):
        '''
        Converts a pdf into a image. Multi-page pdfs are converted to a list of images.
        An image file is converted to a PIL image 
        '''
        images = []
        ...
        if file_extension == ".pdf":
            ...
            raise TypeError(f"This is an unsupported file type. {file_path} needs to be of type [.pdf, .jpg, ,jpeg, .png].")
        return images
    ...
    def _quantize(self,values,n=99):
        ...
        bucket_indices = []
        for value in values:
            index = int((value - min_value) / bucket_width)  # Determine which bucket it belongs to
    ...
    def _apply_ocr(self,image):
        ...
        words, left, top, width, height = data["text"].astype(str),data["left"], data["top"], data["width"], data["height"]
        data['line_block'] = data.apply(lambda row: [row['block_num'], row['line_num']], axis=1).astype(str)

        # filter empty words and corresponding coordinates
        irrelevant_indices = [idx for idx, word in enumerate(words) if not word.strip()]
        words = [word for idx, word in enumerate(words) if idx not in irrelevant_indices]
        line_blocks = [coord for idx, coord in enumerate(data['line_block']) if idx not in irrelevant_indices]
    ...
    def _create_segments(self,image):
        ...
        data['segment'] = data.groupby('lines')['words'].transform(lambda x: ' '.join(map(str, x)))
    ...
    def _parse_entity(self,chunk,value):
        ...
        for i in range(0,len(result)//2):
            ...
            segment_value = chunk[chunk['segment_id'] == entity_segment]['segment'].values[0]

            if entity_value not in segment_value:
                continue
            x_vals.append(chunk[chunk['segment_id'] == entity_segment]['x-mid'].values[0])
            y_vals.append(chunk[chunk['segment_id'] == entity_segment]['y-mid'].values[0])
    ...
    def majority_voting(self,all_responses):
        ...
        for response in all_responses:
            for k, v in response.items():
                ...
                merged[k]['value'] = Counter([v['value'], merged[k]['value']]).most_common(1)[0][0]
        return merged
